...
var BridgeController = class {
	...
	*/
	async execute(command, params, signal) {
		...
		const merged = params.focusPolicy === void 0 ? {
			...params,
			focusPolicy: this.current?.focusPolicy ?? "preserve"
		} : params;
...
*/
const EXPECT_PARAM_DESCRIPTION = "Optional post-condition verified in the SAME call, so you do not need a follow-up screenshot or query to learn whether the action worked. Give exactly one of: {selector} (element appears), {text} (page text appears), {gone} (page text disappears), or {target, value} (an element reaches an exact value). Optional {timeoutMs} (default 5000, max 120000). The result reports expected.matched — a miss is a normal answer, not an error, so false means \"not observed yet\", not \"the call failed\".";
...
function summarizeTabs(value) {
	...
	const lines = shown.map((tab) => {
		...
		const blocked = typeof row.restricted === "string" && row.restricted.length > 0 ? `  [not automatable: ${row.restricted}]` : "";
...
function applyBrowserTools(ctx, controller) {
	ctx.systemPrompt.section({
		name: "tool:browser",
		order: 112,
		text: "The browser_* tools drive the user's real, logged-in browser through the DSH Browser Control extension; they act on the active tab unless a tabId is passed. Prefer browser_snapshot first on unfamiliar pages: it numbers interactive elements, and browser_click/browser_type accept the returned ref instead of guessing CSS selectors. browser_read extracts page text, browser_screenshot saves a PNG/JPEG and returns its file path (view it with an image tool). Calls fail with actionable copy while the bridge is disabled or no browser is connected.\n\nSHARING THE BROWSER WITH THE USER: by default (focusPolicy=preserve) the tools never activate a tab or focus a window, so the user can keep working while you operate. This costs you almost nothing: reads are background-safe, and typing is too — browser_type(mode:\"type\") and browser_press deliver REAL keystrokes to a background tab, because CDP key events do not require the tab to be in front. The one genuine trade-off is clicking: a trusted mouse event lands at viewport coordinates and so needs the tab rendered in front, so preserve dispatches a DOM click and marks the result inputDegraded:\"dom-synthetic\" (isTrusted is false, overlays are not hit-tested). Treat that marker as a signal, not a failure: check the returned state (value, formSubmitted, page change) or pass expect, and only retry with focusPolicy=\"steal\" when a widget provably ignores synthetic clicks — that retry interrupts whatever the user is doing, so prefer reading over re-clicking. browser_tabs activate and an explicit active:true on browser_tabs open are always respected as deliberate foreground requests.\n\nWHAT CANNOT BE AUTOMATED: Chrome blocks the debugger protocol on two classes of page, so no browser_* read or click will ever work on them. This is a browser security boundary, not a transient failure — never retry it: (1) another extension's pages, chrome-extension://<some other id>/… such as Tampermonkey's install or editor UI; (2) browser-internal pages, chrome://… and edge://… . browser_tabs list marks such tabs \"restricted\", and a refused call reports code restricted_other_extension or restricted_browser_internal. Listing, opening, closing and navigating them still work, and their url and title stay readable — only in-page access is blocked. When a task needs one, stop and hand that single step to the user with a concrete instruction, then carry on with the rest.\n\nPICK THE CHEAPEST WAY TO OBSERVE. Measured on real pages, the cost spread is roughly 200x between strategies, so escalate a level only when the one above genuinely cannot answer: (1) browser_evaluate reading exactly the fields you need — ~30 bytes, single-digit ms; (2) browser_read for the page as text — kilobytes; (3) browser_snapshot then browser_click by ref — one round trip for the interactive map, and prefer it over guessing CSS selectors; (4) browser_screenshot LAST — it is the slowest tool (~700ms) and returns a file path you must then open. browser_read(mode:\"html\") is almost always a mistake: it returned 126KB where an evaluate of the same page returned 33 bytes, and it rarely contains anything evaluate cannot reach. Reads and evaluation never disturb the user; prefer them over acting whenever reading suffices.\n\nSTATE THE OUTCOME INSTEAD OF CHECKING LATER: browser_click, browser_type and browser_press accept an optional expect ({selector}|{text}|{gone}|{target,value}) that is verified inside the same call and reported as expected.matched. Use it whenever you would otherwise take a screenshot or issue a separate evaluate to find out whether the action landed. A false expected.matched means \"not observed within the timeout\", not \"the call failed\" — decide from it rather than retrying blindly."
	});
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			focusPolicy: {
				type: "string",
				description: "Per-call override of the plugin's focusPolicy: 'preserve' keeps the user's tab untouched (DOM-synthetic click), 'steal' activates the tab for a trusted mouse event."
			},
			expect: {
				type: "object",
				additionalProperties: false,
				...
				properties: {
					...
					text: {
						type: "string",
						description: "Wait until the page text contains this string."
					},
					gone: {
						type: "string",
						description: "Wait until the page text no longer contains this string."
					},
					target: {
						type: "string",
						description: "With value: the CSS selector whose value is checked."
					},
					value: {
						type: "string",
						description: "With target: the exact value the element must reach."
					},
					timeoutMs: {
						type: "number",
						description: "Budget in ms; default 5000, max 120000."
					}
				}
			}
		},
		...
		async execute(args, exec) {
			...
			if (args.focusPolicy !== void 0) params.focusPolicy = args.focusPolicy;
			if (args.expect !== void 0) params.expect = args.expect;
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			mode: {
				type: "string",
				description: "How the text lands: 'fill' (default) sets the value directly; 'type' replays real key events for keystroke-sensitive widgets but needs OS focus, so under focusPolicy=preserve it degrades to fill and reports inputDegraded."
			},
			focusPolicy: {
				type: "string",
				description: "Per-call override of the plugin's focusPolicy: 'preserve' keeps the user's tab untouched, 'steal' activates the tab so real key events land."
			},
			expect: {
				type: "object",
				additionalProperties: false,
		...
		async execute(args, exec) {
			...
			if (args.tabId !== void 0) params.tabId = args.tabId;
			if (args.mode !== void 0) params.mode = args.mode;
			if (args.focusPolicy !== void 0) params.focusPolicy = args.focusPolicy;
			if (args.expect !== void 0) params.expect = args.expect;
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			focusPolicy: {
				type: "string",
				description: "Per-call override of the plugin's focusPolicy: 'preserve' dispatches an untrusted in-page KeyboardEvent, 'steal' activates the tab for a real key event."
			},
			expect: {
				type: "object",
				additionalProperties: false,
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			timeoutMs: {
				type: "number",
				description: "Wall-clock budget in ms (100-120000). Omit for no client-side deadline; set it when the expression may await something slow."
			}
		},
		output: {
			schema: {
		...
		async execute(args, exec) {
			...
			if (args.tabId !== void 0) evalParams.tabId = args.tabId;
			if (args.timeoutMs !== void 0) evalParams.timeoutMs = args.timeoutMs;
