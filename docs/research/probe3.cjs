...
const PROFILE=path.join(os.tmpdir(),'cdp3-'+Date.now());
...
async function main(){
  ...
  const child=spawn(EDGE,['--remote-debugging-port='+PORT,'--user-data-dir='+PROFILE,
    '--no-first-run','--no-default-browser-check','--window-position=100,100','about:blank'],{stdio:'ignore'});
  let ver=null;
  ...
  console.log('Browser:',ver.Browser,'(headed)');
  ...
  const out={};
  await evB("(()=>{window.__t=[];['mousedown','mouseup','click'].forEach(t=>document.addEventListener(t,e=>window.__t.push({t,trusted:e.isTrusted,x:e.clientX,y:e.clientY,id:e.target&&e.target.id}),true));return 'ok'})()");
  ...
  out.step1_state=await evB("JSON.stringify({active:document.activeElement&&document.activeElement.id})");
